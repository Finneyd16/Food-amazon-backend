...
router.get('/get-products-by-category/:categoryId', async (req, res) => {
    const products = await Product.find({ 'category._id': req.params.categoryId }).sort('name');
    
    if (products.length === 0) return res.status(404).send('No products found for the given category ID.');
    res.send(products);
});
